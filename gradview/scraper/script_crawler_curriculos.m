...
dados = webread('https://www.dac.unicamp.br/sistemas/catalogos/grad/catalogo2020/proposta/sug34.html','ContentType','text/html','CharacterEncoding','auto');
...
if length(dados)==1
    ...
    for j=2:length(blocos_semestre)
        ...
        for ind=1:length(semestre_sugerido)
            creditos_sum=creditos_sum+getcredits(tudo,semestre_sugerido{ind});
            disciplinasusadas{inddisc}=getdisciplina(tudo,semestre_sugerido{ind});
            inddisc = inddisc+1;
        end
